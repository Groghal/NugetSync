...
public static class MegaReportMerger
{
    ...
    public static void MergePackagesReports(string outputsRoot, string outputPath)
    {
        var reportFiles = Directory.EnumerateFiles(outputsRoot, "NugetSync.Packages.tsv", SearchOption.AllDirectories)
            .OrderBy(path => path, StringComparer.OrdinalIgnoreCase)
            .ToList();

        if (reportFiles.Count == 0)
        {
            throw new InvalidOperationException("No packages report files found to merge.");
        }
